...
with st.form("symptom_form"):
    user_input = st.text_area(
        "Write your symptoms (Don't rely fully on the chatbot. See the confidence score for accuracy.)",
        height=100,
        placeholder="e.g. I've had fever and body pain since yesterday."
    )
...
if submitted:
    if not user_input.strip():
        ...
        with st.spinner("🔍 Predicting diseases..."):
            try:
                res = requests.post(
                    "https://anxhu2004-symptom-backend.hf.space/predict",
                ...
                if res.status_code != 200:
                    ...
                    for i, pred in enumerate(data['predictions'], 1):
                        ...
                        st.markdown("**💊 Description & Precautions:**")
                        st.markdown(pred['precautions'], unsafe_allow_html=True)
                        st.markdown(f"**💡 Known Symptoms:** `{', '.join(pred['known_symptoms'])}`")

            except Exception as e:
                st.error("❌ Backend not responding. Please ensure the FastAPI server is live.")
